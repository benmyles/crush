...
func TestProviders_KeepsCatalogWhenCachingFails(t *testing.T) {
	...
	require.Equal(t, catwalk.InferenceProvider("codex"), providers[0].ID, "Codex stays at the front")
	require.Equal(t, catwalk.InferenceProvider("hyper"), providers[1].ID, "Hyper stays at the front of the fetched catalog")
...
func TestProviders_FallsBackToEmbeddedHyper(t *testing.T) {
	...
	require.NoError(t, err)
	require.Len(t, providers, 3)
	require.Equal(t, catwalk.InferenceProvider("codex"), providers[0].ID)
