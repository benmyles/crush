...
type SelectedModel struct {
	...
	ReasoningEffort string `json:"reasoning_effort,omitempty" jsonschema:"description=Reasoning effort level for OpenAI models that support it,enum=none,enum=low,enum=medium,enum=high,enum=xhigh,enum=max"`
...
type Options struct {
	...
	StatusUpdates             bool         `json:"status_updates,omitempty" jsonschema:"description=Enable periodic agent status updates (done, doing, next) shown in the sidebar,default=false"`
	WebBackend                string       `json:"web_backend,omitempty" jsonschema:"description=Backend for web search and web fetching. Default uses Firecrawl when the FIRECRAWL_API_KEY environment variable is set, then Exa when EXA_API_KEY is set, and falls back to DuckDuckGo search and direct HTTP fetching otherwise. Firecrawl forces the Firecrawl API. Exa forces the Exa API.,enum=default,enum=firecrawl,enum=exa,default=default"`
...
type Config struct {
	...
	Hooks map[string][]HookConfig `json:"hooks,omitempty" jsonschema:"description=User-defined shell commands that fire on hook events (e.g. PreToolUse, SessionStart)"`
...
func resolveReadOnlyTools(tools []string) []string {
	readOnlyTools := []string{"glob", "grep", "ls", "lsp_call_hierarchy", "lsp_definition", "lsp_symbols", "sourcegraph", "view", "recall_grep", "recall_describe", "recall_expand"}
