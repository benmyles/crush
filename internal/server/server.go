...
func NewServer(cfg *config.ConfigStore, network, address string) *Server {
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/plans/respond", c.handlePostWorkspacePlanResponse)
	mux.HandleFunc("POST /v1/workspaces/{id}/user_questions/respond", c.handlePostWorkspaceUserQuestionResponse)
	...
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/compact", c.handlePostWorkspaceAgentSessionCompact)
	mux.HandleFunc("POST /v1/workspaces/{id}/agent/sessions/{sid}/compact-for-plan", c.handlePostWorkspaceAgentSessionCompactForPlan)
	mux.HandleFunc("POST /v1/workspaces/{id}/jobs/{shell_id}/input", c.handlePostWorkspaceJobInput)
	mux.HandleFunc("POST /v1/workspaces/{id}/jobs/{shell_id}/resize", c.handlePostWorkspaceJobResize)
	...
	mux.HandleFunc("GET /v1/workspaces/{id}/config/critical-instructions", c.handleGetWorkspaceConfigCriticalInstructions)
	mux.HandleFunc("GET /v1/workspaces/{id}/config/snippets", c.handleGetWorkspaceConfigSnippets)
