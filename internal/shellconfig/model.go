...
// "small", and "compaction" set the selected model for that slot, or print
// the current selection as <provider>/<id> when given no argument. The
// compaction slot is optional: when unset, context compaction runs on the
// large model.
func handleModel(ctx context.Context, args []string, stdin io.Reader, stdout, stderr io.Writer) error {
	...
	if len(args) < 2 {
		return usage(stderr, "usage: model add|remove <provider>/<id> | model large|small|compaction [<provider>/<id>]")
	}
	...
	case "large", "small", "compaction":
		return modelSelect(b, args, stdout, stderr)
	default:
		return usage(stderr, fmt.Sprintf("model: unknown subcommand %q (expected add, remove, large, small, or compaction)", args[1]))
	}
}
