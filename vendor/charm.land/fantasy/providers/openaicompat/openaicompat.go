...
func WithDisableMaxOutputTokens() Option {
	return func(o *options) {
		o.openaiOptions = append(o.openaiOptions, openai.WithDisableMaxOutputTokens())
	}
}

// WithUseWebSocketTransport configures Responses API models to stream over
// a WebSocket connection, falling back to SSE when the connection fails.
// Intended for backends that serve both transports, like ChatGPT Codex.
func WithUseWebSocketTransport() Option {
	return func(o *options) {
		o.openaiOptions = append(o.openaiOptions, openai.WithUseWebSocketTransport())
	}
}
