...
type options struct {
	...
	// disableMaxOutputTokens suppresses the max_output_tokens request
	// parameter entirely. Some backends (notably the ChatGPT Codex
	// subscription endpoint) reject the field outright.
	disableMaxOutputTokens bool

	// useWebSocketTransport streams Responses API requests over a
	// WebSocket (response.create frames) with SSE as a fallback. It is
	// intended for the ChatGPT Codex backend, which serves both.
	useWebSocketTransport bool

	// websocketURL is the WebSocket endpoint for Responses streams.
	// When empty it is derived from baseURL.
	websocketURL string

	// websocketHeaders are extra headers applied to WebSocket
	// connections. The default Responses request headers are reused when
	// this is empty.
...
// WithDisableMaxOutputTokens configures the provider to omit the
// max_output_tokens request parameter for every call. Backends that reject
// the field (e.g. the ChatGPT Codex subscription endpoint) need this.
func WithDisableMaxOutputTokens() Option {
	return func(o *options) {
		o.disableMaxOutputTokens = true
	}
}

// WithUseWebSocketTransport configures Responses API models to stream over
// a WebSocket connection, falling back to SSE when the connection or the
// handshake fails. The WebSocket URL defaults to the responses endpoint
// derived from the base URL.
func WithUseWebSocketTransport() Option {
	return func(o *options) {
		o.useWebSocketTransport = true
	}
}

// WithWebSocketURL sets the WebSocket endpoint used by
// WithUseWebSocketTransport. By default it is derived from the base URL.
...
// WithWebSocketHeaders adds headers to WebSocket connection handshakes.
func WithWebSocketHeaders(headers map[string]string) Option {
	return func(o *options) {
		if o.websocketHeaders == nil {
			o.websocketHeaders = make(map[string]string)
		}
		maps.Copy(o.websocketHeaders, headers)
	}
}
...
func (o *provider) LanguageModel(_ context.Context, modelID string) (fantasy.LanguageModel, error) {
	...
	if o.options.useResponsesAPI && o.isResponsesModel(modelID) {
		...
		return newResponsesLanguageModel(modelID, o.options.name, client, objectMode, responsesTransportOptions{
			websocketEnabled: o.options.useWebSocketTransport,
			websocketURL:     o.options.websocketURL,
			websocketHeaders: o.options.websocketHeaders,
			disableMaxOutput: o.options.disableMaxOutputTokens,
			baseURL:          o.options.baseURL,
			apiKey:           o.options.apiKey,
			requestHeaders:   resolved,
		}), nil
	}
